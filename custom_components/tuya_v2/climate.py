...
# A list of device categories that can be classified as purely heaters - allows for future expansion
TUYA_HEATER_TYPE = {
    "qn",  # Heater
}
...
class TuyaHaClimate(TuyaHaDevice, ClimateEntity):
    ...
    def set_hvac_mode(self, hvac_mode):
        ...
        # Only pass the DPCODE_MODE command if the device supports it
    ...
    def hvac_mode(self) -> str:
        ...
        # If the switch is NOT off and the device does not offer any HVAC modes, assume it's HEAT if it's a heater or HEAT/COOL otherwise
        if DPCODE_MODE not in self.tuya_device.status:
            _LOGGER.debug(f"Device has switch turned on but supports no modes so just returning default mode")
            if self.tuya_device.category in TUYA_HEATER_TYPE:
    ...
    def hvac_modes(self) -> list:
        ...
        if DPCODE_MODE not in self.tuya_device.function:
            # If the device doesn't support any modes, but does support switch, just return the OFF and HEAT modes if it's a heater or OFF and HEAT/COOL otherwise to allow the UI to function
            if self.tuya_device.category in TUYA_HEATER_TYPE:
                return [] if DPCODE_SWITCH not in self.tuya_device.function else [HVAC_MODE_OFF, HVAC_MODE_HEAT]
            return [] if DPCODE_SWITCH not in self.tuya_device.function else [HVAC_MODE_OFF, HVAC_MODE_HEAT_COOL]
